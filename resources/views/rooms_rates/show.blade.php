<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="max-w-lg flex flex-col justify-center items-center py-4 lg:max-w-none lg:p-6 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="flex flex-col justify-center items-start gap-2">
                    <x-item-p label="Название: " value="{{$rate->name}}"></x-item-p>
                    <x-item-p label="Стоимость за час: " value="{{$rate->price}}"></x-item-p>
    ...
    <x-modal name="confirm-rooms_rate-deletion" focusable>
        <form method="post" action="{{ route('rooms_rates.destroy', $rate->id) }}" class="p-6">
            @csrf
            @method('delete')

            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                {{ __('Вы уверены, что хотите удалить тариф "' . $rate->name . '" и все комнаты с этим тарифом?') }}
            </h2>
